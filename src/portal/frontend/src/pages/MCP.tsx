  ...
  return (
    <>
      <Card>
        ...
        {connection.authMode === 'oauth' && <section className="space-y-2 lg:col-span-3"><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-medium">Upstream OAuth</h3><Badge variant={oauthStatus?.authorized ? 'secondary' : 'outline'}>{oauthStatus?.authorized ? 'authorized' : oauthStatus?.configured ? 'client configured' : 'not configured'}</Badge>{oauthStatus?.expiresAt && <span className="text-xs text-muted-foreground">expires {new Date(oauthStatus.expiresAt).toLocaleString()}</span>}</div>{oauthStatus?.issuer && <p className="text-xs text-muted-foreground">{oauthStatus.issuer}{oauthStatus.scopes.length ? ` · ${oauthStatus.scopes.join(' ')}` : ''}</p>}<div className="grid gap-2 sm:grid-cols-2"><Input value={issuer} onChange={(e) => setIssuer(e.target.value)} placeholder="Issuer (optional; discovered when blank)" /><Input value={clientId} onChange={(e) => setClientId(e.target.value)} placeholder="Client ID" /><Input type="password" value={clientSecret} onChange={(e) => setClientSecret(e.target.value)} placeholder="Client secret (optional)" /><Input value={scopes} onChange={(e) => setScopes(e.target.value)} placeholder="Space-separated scopes" /></div><div className="flex gap-2"><Button size="sm" disabled={!clientId} onClick={() => api.configureMcpOAuth(connection.id, { issuer: issuer || undefined, clientId, clientSecret: clientSecret || undefined, scopes: scopes.split(/\s+/).filter(Boolean) }).then(load).catch(onError)}>Save OAuth client</Button><Button size="sm" variant="outline" disabled={!oauthStatus?.configured} onClick={() => api.connectMcpOAuth(connection.id).then((result) => { window.location.href = result.authorizeUrl }).catch(onError)}>Authorize</Button><Button size="sm" variant="ghost" disabled={!oauthStatus?.configured} onClick={() => api.deleteMcpOAuth(connection.id).then(load).catch(onError)}>Remove OAuth</Button></div></section>}
        </CardContent>
      </Card>
